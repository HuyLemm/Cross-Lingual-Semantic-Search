...
def split_long_sentence(sentence: str) -> List[str]:
    """
    Split very long sentence into clauses by separators (keeps semantics better).
    """
    s = (sentence or "").strip()
    if not s:
        return []
...
def build_chunks_from_sentences(
    ...
    """
    Sentence-aligned chunking with soft char budget.
    Returns:
      chunks: list[str]
      metas: list[dict] (sent_start/sent_end, sent_count, char_len)
      stats: dict
    """
    chunks: List[str] = []
...
def _tfidf_token_pattern() -> str:
    """
    IMPORTANT CHANGE:
    - Old pattern only kept letters => dropped years/dates/acronyms with digits.
    - New pattern keeps:
        2020
        12/05/2020
        2020-05-12
        Q3-2024
        Nobel, FIFA, Hà_Nội (underscores are ok)
    """
...
def build_doc_and_chunk_keywords(
    ...
    """
    Fit TF-IDF on: all chunks + [doc]
    - doc_keywords from last row
    - chunk_keywords from each chunk row
    """
...
def build_existing_cache_map(cache_dir: str) -> Dict[str, str]:
    """
    Map source_id -> out_dir for valid caches.
    """
...
def generate_index_for_file(
    ...
    try:
        ...
        embedder = STEmbeddings(st_model)
        ...
        meta_index = {
            ...
            "note": (
                "Short sentence-aligned chunks to reduce semantic averaging; "
                "stores doc_keywords + chunk_keywords in FAISS metadata for lexical gate/hybrid scoring; "
                "TITLE excluded from embedding to avoid title-driven false positives; "
                "TF-IDF token pattern keeps digits/date-like tokens."
            ),
        }
...
def main():
    ...
    st_model = SentenceTransformer(opts.embed_model_name)
