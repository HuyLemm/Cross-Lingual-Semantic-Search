...
type PdfMeta = {
  ...
  pdfUrl: string; // "/qa/pdf?...."
  downloadUrl: string; // "/qa/pdf?....&download=1"
};

interface QAPairPDFViewerModalProps {
  qa: QAPair;
  meta: PdfMeta | null;
  loading?: boolean;
  error?: string | null;
...
function formatPages(pages?: number | null) {
  ...
  if (pages === undefined || pages === null || pages <= 0) return "—";
  return String(pages);
}

export default function QAPairPDFViewerModal({
  qa,
  meta,
  loading = false,
  error = null,
  onClose,
}: QAPairPDFViewerModalProps) {
  const pdfName = meta?.pdfName ?? qa.sourceDocument ?? "Unknown.pdf";

  const sizeText =
    meta?.sizeBytes !== undefined ? formatBytes(meta.sizeBytes) : "—";
  const pagesText = formatPages(meta?.pages);

  const API_BASE = "http://localhost:4000";
  const pdfSrc = meta?.pdfUrl ? `${API_BASE}${meta.pdfUrl}` : "";
  const downloadHref = meta?.downloadUrl ? `${API_BASE}${meta.downloadUrl}` : "";
